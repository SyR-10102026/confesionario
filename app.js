...
async function saveRecord(file,duration){
 const blob=file.slice(0,file.size,file.type||'video/webm');const id=Date.now();const created=new Date().toISOString();
 app.innerHTML=`<div class="processing"><div><h1>Guardando vuestra confesión…</h1><p>Un momento, por favor.</p></div></div>`;
 try{
  await putVideo({id,blob,mode:currentMode||'libre',duration:Math.round(duration),created});
  downloadLocal(blob,id,file.type||'video/webm');
  let remote=null;
  if(SUPABASE_ANON_KEY!=='PEGA_AQUI_TU_ANON_KEY') remote=await uploadToSupabase(blob,id,file.type||'video/webm');
  showThanks(remote);
 }catch(e){console.error(e);toast('No se pudo guardar la confesión.');setTimeout(()=>finish(),1800)}
}
function downloadLocal(blob,id){
 const ext=(blob.type||'').includes('mp4')?'mp4':'webm';const u=URL.createObjectURL(blob);const a=document.createElement('a');a.href=u;a.download=`confesion-${id}.${ext}`;a.rel='noopener';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),5000);
}
async function uploadToSupabase(blob,id,type){
 const fd=new FormData();fd.append('video',blob,`confesion-${id}.${type.includes('mp4')?'mp4':'webm'}`);
 ...
 const data=await r.json();if(!r.ok||!data.success)throw new Error(data.error||`HTTP ${r.status}`);return data;
}
function showThanks(remote){
 app.innerHTML=bg(6)+`<div class="thanks-status">${remote?'<p class="upload-ok">✓ Copia temporal preparada</p>':'<p class="upload-warn">⚠ Falta configurar la clave de Supabase</p>'}</div><button class="hotspot home-start" aria-label="Volver al inicio" onclick="home()"></button>`;
 setTimeout(home,12000);
}
